<?php
    ...
    if ($action == 'validateTx') {
        ...
        // Query PoS RPC for transaction status
        $rpc_data = array(
            "jsonrpc" => "2.0",
        ...
        $ch = curl_init($nimiq_rpc);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($rpc_data));
        ...
        if (isset($result['result']) && $result['result'] !== null) {
            ...
            // Check if transaction has enough confirmations (PoS typically needs fewer confirmations)
            if ($tx_data['confirmations'] >= 2) { // Requiring 2 confirmations for PoS
                $tx_status = "confirmed";
        ...
        if ($tx_status == 'confirmed') {
            // get invoice type
            $invoice = DB::queryFirstRow("SELECT type FROM nimipay_invoices WHERE id_invoice=%s", $id_invoice);
            $invoice_type = $invoice['type'];

            if ($invoice_type == 'demo_invoice') {
